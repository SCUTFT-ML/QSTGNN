...
def make_quaternion_mul(kernel):
    ...
    dim = kernel.size(2)//4
    r, i, j, k = torch.split(kernel, [dim, dim, dim, dim], dim=2)
    ...
    hamilton = torch.cat([r2, i2, j2, k2], dim=2)
    assert kernel.size(2) == hamilton.size(2)
    return hamilton

"""@Chaoxiong Lin"""
'''Make a Hamilton matrix for quaternion GCN adj'''
def make_quaternion_GCN_adj_mul(gso1, gso2, gso3, gso4, x):
    """" gso is adj, x is the data """
    r, i, j, k = gso1, gso2, gso3, gso4
    r2 = torch.cat([r, -i, -j, -k], dim=1)  # 0, 1, 2, 3
    i2 = torch.cat([i, r, -k, j], dim=1)  # 1, 0, 3, 2
    j2 = torch.cat([j, k, r, -i], dim=1)  # 2, 3, 0, 1
    k2 = torch.cat([k, -j, i, r], dim=1)  # 3, 2, 1, 0
    hamilton = torch.cat([r2, i2, j2, k2], dim=0)
    x_hid1 = torch.split(x, split_size_or_sections=x.size(1)//4, dim=1)
    x_hid2 = torch.cat(x_hid1, dim=2)
    x_out1 = torch.einsum('hi,bjit->bjht', hamilton, x_hid2)
    x_out2 = torch.split(x_out1, split_size_or_sections=x_out1.size(2)//4, dim=2)
    x_out3 = torch.cat(x_out2, dim=1)
    return x_out3

"""@Chaoxiong Lin"""
'''Make a Hamilton matrix for quaternion GCN adj'''
def Quaternion_mul(M1, M2):
    """" gso is adj, x is the data """
    r, i, j, k = torch.split(M1, split_size_or_sections=M1.size(1)//4, dim=1)
    r2 = torch.cat([r, -i, -j, -k], dim=1)  # 0, 1, 2, 3
    i2 = torch.cat([i, r, -k, j], dim=1)  # 1, 0, 3, 2
    j2 = torch.cat([j, k, r, -i], dim=1)  # 2, 3, 0, 1
    k2 = torch.cat([k, -j, i, r], dim=1)  # 3, 2, 1, 0
    hamilton = torch.cat([r2, i2, j2, k2], dim=0)
    Out = torch.mm(hamilton, M2.transpose(1,0))
    return Out
...
class QGNNLayer(Module):
    def __init__(self, in_features, out_features, dropout=0.05, quaternion_ff=True, act=F.relu):
    ...
    def forward(self, input, adj, double_type_used_in_graph=False):
        ...
        if self.quaternion_ff:
            ...
            support = torch.einsum('hi,btij->bthj', self.adj, x)
        else:
